...
class Appointment(models.Model):
    ...
    patient_id = fields.Many2one('lab.patient', string='Patient', required=True, select=False,
                                 help='Patient Name')
    ...
    physician_id = fields.Many2one(
        'res.partner', string='Referred By', select=True)
    comment = fields.Text(string='Comments')
    appointment_lines = fields.One2many(
        'lab.appointment.lines', 'test_line_appointment', string="Test Request")

    request_count = fields.Integer(
        compute="_compute_state", string='# of Requests', copy=False, default=0)
    inv_count = fields.Integer(
        compute="_compute_state", string='# of Invoices', copy=False, default=0)
    ...
    def action_request(self):
        if self.appointment_lines:
            for line in self.appointment_lines:
                data = self.env['lab.test'].search(
                    [('lab_test', '=', line.lab_test.lab_test)])
...
class LabAppointmentLines(models.Model):
    ...
    test_line_appointment = fields.Many2one(
        'lab.appointment', string="Appointment")
...
class LabPatientInherit(models.Model):
    ...
    app_count = fields.Integer(
        compute="_compute_state", string='# of Appointments', copy=False, default=0)

    def _compute_state(self):
        for obj in self:
            obj.app_count = self.env['lab.appointment'].search_count(
                [('patient_id', '=', obj.id)])
